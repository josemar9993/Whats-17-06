...
module.exports = {
  ...
  async execute(message) {
    try {
      ...
      logger.info(
        `Estatísticas geradas em ${duration}ms (cache: ${duration < 50 ? 'HIT' : 'MISS'})`
      );
  ...
  async calculateStats() {
    return await retryManager.withDatabaseRetry(async () => {
      ...
      totalGrupos = Array.from(chatIds).filter((id) =>
        id.endsWith('@g.us')
      ).length;
      totalUsuarios = Array.from(
        new Set(allMsgs.map((m) => m.senderName || m.contactName || m.from))
      ).length;

      // Estatísticas adicionais
      const today = new Date().toISOString().slice(0, 10);
      const todayMsgs = allMsgs.filter((m) =>
        m.isoTimestamp?.startsWith(today)
      );
