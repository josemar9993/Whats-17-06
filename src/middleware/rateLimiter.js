...
class RateLimiter {
  ...
  checkLimit(userId, commandName = 'unknown') {
    ...
    if (validRequests.length >= limit) {
      ...
      if (validRequests.length > limit * 2) {
        ...
        logger.warn(
          `Usuário ${userId} bloqueado temporariamente por excesso de requisições`,
          {
    ...
    if (validRequests.length > limit * 0.8) {
      // Avisar quando chegar a 80% do limite
  ...
  cleanup() {
    ...
    if (cleanedUsers > 0 || cleanedBlocks > 0) {
      logger.debug(
        `Rate limiter cleanup: ${cleanedUsers} usuários, ${cleanedBlocks} bloqueios`
      );
    }
  }
  ...
  getStats() {
    ...
    const stats = {
      ...
      blockedUsers: Array.from(this.blockedUsers.entries()).filter(
