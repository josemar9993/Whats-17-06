...
class ErrorHandler {
  ...
  isCritical(error) {
    return this.criticalErrors.some(
      (criticalError) =>
      ...
      if (global.whatsappClient) {
        for (const adminId of adminIds) {
          try {
            ...
            logger.error(
              `Erro ao enviar notificação para admin ${adminId}:`,
              sendError
            );
          }
        }
  ...
  getErrorResponse(errorInfo) {
    switch (errorInfo.type) {
      ...
      case 'RATE_LIMIT_ERROR':
        return {
          userMessage:
            '⏰ Você está enviando comandos muito rapidamente. Aguarde um momento.',
          shouldReply: true
        };
      ...
      case 'DATABASE_ERROR':
        return {
          userMessage:
            '🛠️ Erro temporário no sistema. Tente novamente em alguns segundos.',
          shouldReply: true
        };
      ...
      case 'TIMEOUT':
        return {
          userMessage:
            '⏱️ Operação demorou muito para responder. Tente novamente.',
          shouldReply: true
        };
      ...
      default:
        if (errorInfo.severity === 'CRITICAL') {
          return {
            userMessage:
              '🚨 Erro crítico detectado. Os administradores foram notificados.',
            shouldReply: true
          };
        }
