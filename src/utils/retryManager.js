  ...
  getStats() {
    ...
    for (const [context, stats] of this.retryStats) {
      summary.contexts[context] = {
        ...
        avgAttempts:
          stats.totalOperations > 0
            ? (stats.totalAttempts / stats.totalOperations).toFixed(2)
            : 0
      };
    ...
    if (summary.overall.totalOperations > 0) {
      const successfulOps = Array.from(this.retryStats.values()).reduce(
        (sum, stats) => sum + stats.successfulOperations,
        0
      );

      summary.overall.successRate = (
        (successfulOps / summary.overall.totalOperations) *
        100
      ).toFixed(2);
      summary.overall.avgAttemptsPerOperation = (
        summary.overall.totalAttempts / summary.overall.totalOperations
      ).toFixed(2);
    }
